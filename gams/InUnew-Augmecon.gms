...
*-*-*-*-*-*-*-*-*-*-*-* Set *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
sets
   j index for distribution center /1*15/
   i index for customer point /1*15/
   c index for collection center /1*15/
...
scalars
   ...
   Gamma fixed value for cannibalization effect in linear model /0.1/
...
parameter
    pd(i)                percent of discount
;
    loop((i),
    pd(i)=0.2
    );
parameter
    TainU(i)         CE inverted-u;
    loop((i),
    TainU(i)=-0.0391+6.3151*(pd(i))-21.4636*((pd(i))**2)+24.9582*((pd(i))**3)-9.7866*((pd(i))**4);
    );
...
obj11    ..    Z1=e=sum((i),(pn(i)-Pcn-Prwn)*XX*mn(i)*(1-(Landa*pn(i)))*(1-TainU(i)))-
                   sum((i,j),Beta*XX*Tdcn(i,j)*eD(i,j)*Yn(i,j)*mn(i)*(1-(Landa*pn(i)))*(1-TainU(i)))-
                   sum(j,An(j)*(XX/tn(j))*X(j))-sum(j,Beta*Tfcn(j)*(XX/tn(j))*X(j))-
                   sum((i,j),Beta*XX*Tvcn(j)*eM(j)*Yn(i,j)*mn(i)*(1-(Landa*pn(i)))*(1-TainU(i)))-
                   sum((i,j),0.5*teta*Hn*tn(j)*Yn(i,j)*mn(i)*(1-(Landa*pn(i)))*(1-TainU(i)))-
                   sum(j,((Pain(j)*XX)/tn(j))*phiztn(j)*c1(j))+
                   sum((i),(pr(i)-Pcr-Prwr)*XX*mr(i)*(1-(Landa*pr(i))))+
                   sum((i),(pr(i)-Pcr-Prwr)*XX*mn(i)*(1-(Landa*pn(i)))*TainU(i))-
                   sum((i,j),Beta*XX*Tdcr(i,j)*eD(i,j)*Yr(i,j)*mr(i)*(1-(Landa*pr(i))))-
                   sum((i,j),Beta*XX*Tdcr(i,j)*eD(i,j)*Yr(i,j)*mn(i)*(1-(Landa*pn(i)))*TainU(i))-
                   sum(j,Ar(j)*(XX/tr(j))*X(j))-sum(j,Beta*Tfcr(j)*(XX/tr(j))*X(j))-
                   sum((i,j),Beta*XX*Tvcr(j)*eM(j)*Yr(i,j)*mr(i)*(1-(Landa*pr(i))))-
                   sum((i,j),Beta*XX*Tvcr(j)*eM(j)*Yr(i,j)*mn(i)*(1-(Landa*pn(i)))*TainU(i))-
                   sum((i,j),0.5*teta*Hr*tr(j)*Yr(i,j)*mr(i)*(1-(Landa*pr(i))))-
                   sum((i,j),0.5*teta*Hr*tr(j)*Yr(i,j)*mn(i)*(1-(Landa*pn(i)))*TainU(i))-
...
obj21    ..   Z2=e=sum((i,j),eI(j)*eM(j)*XX*Yn(i,j)*mn(i)*(1-(Landa*pn(i)))*(1-TainU(i)))+
                  sum((i,j),eI(j)*eM(j)*XX*Yr(i,j)*mr(i)*(1-(Landa*pr(i))))+
                  sum((i,j),eI(j)*eM(j)*XX*Yr(i,j)*mr(i)*(1-(Landa*pn(i)))*TainU(i))+
                  sum((i,j),eId(i,j)*eD(i,j)*XX*Yn(i,j)*mn(i)*(1-(Landa*pn(i)))*(1-TainU(i)))+
                  sum((i,j),eId(i,j)*eD(i,j)*XX*Yr(i,j)*mr(i)*(1-(Landa*pr(i))))+
                  sum((i,j),eId(i,j)*eD(i,j)*XX*Yr(i,j)*mn(i)*(1-(Landa*pn(i)))*TainU(i))+
...
eq65(j)..       (C1(j)**2)=g=sum(i,(tn(j)+Ln(j))*(Sigman1(i)**2)*Yn(i,j)*(mn(i)**2)*((1-(Landa*pn(i)))**2)*((1-TainU(i))**2))+sum(i,(tn(j)+Ln(j))*(Sigman2(i)**2)*Yn(i,j));
eq66(j)..       (C2(j)**2)=g=sum(i,(tr(j)+Lr(j))*(Sigmar1(i)**2)*Yr(i,j)*(mr(i)**2)*((1-(Landa*pr(i)))**2))+sum(i,(tr(j)+Lr(j))*(Sigmar1(i)**2)*Yr(i,j)*(mn(i)**2)*((1-(Landa*pn(i)))**2)*(TainU(i)**2))+sum(i,(tr(j)+Lr(j))*(Sigmar1(i)**2)*Yr(i,j)*2*mr(i)*mn(i)*(1-(Landa*pn(i)))*(1-(Landa*pr(i)))*TainU(i))+sum(i,(tr(j)+Lr(j))*(Sigmar2(i)**2)*Yr(i,j));
eq67(i,j)..      X(j)=g=Yn(i,j);
eq68(i,j)..      X(j)=g=Yr(i,j);
eq69(i)..        sum(j,Yn(i,j))=e=1;
eq70(i)..        sum(j,Yr(i,j))=e=1;
eq71(i,j)..      pn(i)=g=(Tvcn(j)+Tfcn(j)+Prwn+Pcn)*Yn(i,j);
eq72(i,j)..      pr(i)=g=(Tvcr(j)+Tfcr(j)+Prwr+Pcr)*Yr(i,j);
eq73(i)..        pr(i)=l=pn(i);
eqq73(i)..       pn(i)=l=(1/Landa);
eq74(i,c)..      vu(i,c)=l=pr(i);
eq75(i,c)..      vu(i,c)=l=Rp(i,c)-Trcu(i,c);
eqq75(i,c)..     svu(i)=l=vu(i,c);
eq76(i)..        sum(c,Tpu(i,c))=e=1;
eq77(i,c)..      Tpu(i,c)=l=Tpl(c);
eq78(i)..        (pn(i)-pr(i))=e=pd(i)*pn(i);
eq79(i)..         mn(i)*(1-(Landa*pn(i)))*(1-TainU(i))=g=1;
